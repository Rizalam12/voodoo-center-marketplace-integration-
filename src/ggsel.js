...
function buildWebhookNotificationSettings(){const url=process.env.GGSEL_WEBHOOK_URL?.trim();if(!url)throw Error("GGSEL_WEBHOOK_URL is missing.");if(!/^https:\/\//i.test(url))throw Error("GGSEL_WEBHOOK_URL must be a public HTTPS URL.");return{type:"url",url,http_method:"POST",is_disabled:false,is_default:false};}
...
async function syncOfferStock(offerId,stockQuantity){const quantity=Math.max(0,Math.floor(Number(stockQuantity)||0));return patchOffer(offerId,{quantity,is_unlimited_quantity:false});}
module.exports={testConnection,getCategories,searchCategories,createOffer,getOffer,patchOffer,buildWebhookNotificationSettings,getPurchaseInfo,selectTestProduct,buildOfferPayload,validateOfferPayload,sanitizeProduct,dryRun,recordNotification,responseData,syncOfferStock};
